...
trait OrderDaoT {
  ...
  def addOrder(order: Order): Order = {
    DB.withConnection { implicit c =>
      ...
      for(orderItem <- order.orderItems) {
        val orderItemID: Option[Long] =
        SQL("insert into order_items(order_id, item_id, item_name, size, quantity, costs) values ({order_id}, {item_id}, {item_name}, {size}, {quantity}, {costs});").on(
            'order_id -> order.id, 'item_id -> orderItem.id, 'item_name -> orderItem.name, 'size -> orderItem.size, 'quantity -> orderItem.quantity, 'costs -> orderItem.price).executeInsert()
      }
    }
    order
  }
  ...
  def getOrdersByCustID(custID: Long): List[Order] = {
    DB.withConnection { implicit c =>
      val selectOrdersByCustID= SQL("Select id, order_date, costs from Orders where cust_id = {id};").on('id -> custID)
